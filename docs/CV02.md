...
2. **Ruční implementace:**
   - Vytvořte **gettery, settery a konstruktor** ručně.
   ...
   - Upravte metodu **sayGreeting**, aby logovala vytvořenou instanci a zároveň jí vrátila v textové podobě jako odpověď.
   - **Otestujte funkčnost implementace** spuštěním aplikace a kontrolou výpisu logů.

3. **Refaktorování s Lombokem:**
   - Přepište třídu User tak, aby používala anotace z **Lomboku** ( `@AllArgsConstructor`, `@Setter`, `@Getter`).
...
- Upravte endpoint `/user` aby přijal ID uživatele jako **Query parameter** (`/users?id=1`) a vrátí odpovídající instanci třídy `User`.
- Otestujte endpoint pomocí **Postmanu**.
