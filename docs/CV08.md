...
### 1. Příparava backendu

- Do entity `User.java` přidejte atribut `active` typu `boolean` pokud již nemáte.
...
- Deaktivujte ochranu CORS.
- Spusťte Spring Boot.

### 2. Komunikace SPA s REST API

- Upravte kód z cvičení `CV07` tak, aby místo pole předem definovaných uživatelů byla použita komunikace s backendem.
...
- Proč bychom neměli ochranu CORS vyřadit v produkčním prostředí?
- Jak nastavit Spring Boot aby CORS vypnul jen pro vývojové prostředí?
- V čem je Fetch API odlišné od AXIOS API? Jaké jsou výhody a nevýhody obou přístupů?
- V čem výhodné použít závislost Tanstack Query?

### 5. POST Request ![AI](https://img.shields.io/badge/AI-yellow) ![Dobrovolné](https://img.shields.io/badge/Dobrovolný_úkol-blue)

- Vytvořte dva nové POST endpointy v `UserController.java` které budou měnit stav `active` uživatele:
...
### 6. TanstackQuery ![AI](https://img.shields.io/badge/AI-yellow) ![Dobrovolné](https://img.shields.io/badge/Dobrovolný_úkol-blue)

- Vložte závislost na knihovnu [TanstackQuery](https://tanstack.com/query/latest/docs/framework/react/installation) do projektu.
- Upravte kód z předchozího úkolu tak, společně s AXIOS byla použity hooky TanstackQuery.
